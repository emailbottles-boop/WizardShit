...
(function () {
  ...
  function card(p) {
    ...
    el.innerHTML =
      ...
      '</div>' +
      '<div class="acts">' +
        (p.kind === 'audio' || !p.image || /\.gif$/i.test(p.image || '') ? '' : '<button data-act="trim">Trim</button>') +
  ...
  $('scan').onclick = function () {
    var ids = Object.keys(cards).filter(function (id) { return cards[id].p.kind !== 'audio' && cards[id].p.image && !/\.gif$/i.test(cards[id].p.image || ''); });
