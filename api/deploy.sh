...
echo "    npx wrangler secret put PRINTFUL_TOKEN     # the shop: products, prices, orders"
echo "    npx wrangler secret put STRIPE_SECRET_KEY  # the shop + donations: taking payment"
echo "    npx wrangler secret put STRIPE_WEBHOOK_SECRET  # the shop: hearing back from Stripe"
echo ""
echo "    First time the shop is deployed, also run the one-off column migration:"
echo "    npx wrangler d1 execute wizardshit --remote --file=upgrade-shop.sql"
